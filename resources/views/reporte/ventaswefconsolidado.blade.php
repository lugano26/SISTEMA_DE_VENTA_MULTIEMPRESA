		...
		<div class="nav-tabs-custom">
			<div class="tab-content">
				<div class="tab-pane active" id="tab_1-1">
					<div class="row">
						<div class="col-md-12">
							<form id="frmFiltro" method="post" action="{{url('reporte/ventaswefconsolidado')}}">
								<div class="row">
									<div class="col-md-4">
								...
								<div class="row">
									<div class="form-group col-md-4">
										...
										<select id="selectCategoriaVentaNivelUno" name="selectCategoriaVentaNivelUno" class="form-control" style="display: inline-block;width: 100%;"
										 onchange="onChangeSelectCategoriaVenta('selectCategoriaVentaNivelUno');">
									...
									<div class="form-group col-md-4">
										<label>&nbsp;</label>
										<select id="selectCategoriaVentaNivelDos" name="selectCategoriaVentaNivelDos" class="form-control" style="display: inline-block;width: 100%;"
										 onchange="onChangeSelectCategoriaVenta('selectCategoriaVentaNivelDos');">
											<option class="optionClearCategoriaVenta"></option>
											@foreach($listaTCategoriaVenta as $value)
											@if(!$value->estado) @continue; @endif
											@foreach($value->tcategoriaventachild as $item)
											@if(!$item->estado) @continue; @endif
											<option {{(old('selectCategoriaVentaNivelDos')==$item->codigoCategoriaVenta ? 'selected' : '')}} class="option{{$value->codigoCategoriaVenta}}"
											 value="{{$item->codigoCategoriaVenta}}" style="display: none;">{{$item->descripcion}}</option>
											@endforeach
											@endforeach
										</select>
									</div>
									<div class="form-group col-md-4">
										...
										<select id="selectCategoriaVentaNivelTres" name="selectCategoriaVentaNivelTres" class="form-control" style="display: inline-block;width: 100%;">
											...
											<option {{(old('selectCategoriaVentaNivelTres')==$v->codigoCategoriaVenta ? 'selected' : '')}} class="option{{$item->codigoCategoriaVenta}}"
											 value="{{$v->codigoCategoriaVenta}}" style="display: none;">{{$v->descripcion}}</option>
											@endforeach
											@endforeach
											@endforeach
										</select>
...
<script src="{{asset('viewResources/reporte/ventaswefconsolidado.js?x='.env('CACHE_LAST_UPDATE'))}}"></script>