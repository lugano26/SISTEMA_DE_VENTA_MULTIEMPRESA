					...
					<div class="row clear">
						<div class="col-md-12">
							<div>
								...
								<form id="fmrPagoLetra" class="frmPagoLetra" method="POST" action="{{url('reciboventaletra/realizarpagoletrasinfe')}}">
									{{csrf_field()}}
									<div class="form-group">
										<div class="input-group input-group-sm">
											<input type="hidden" name="codigoReciboVenta" value="{{$tReciboVenta->codigoReciboVentaOutEf }}">
						...
						<div class="col-md-12">
							...
							<table class="table table-striped" style="min-width: 777px;">
								<tbody>
									...
									@if(isset($tReciboVenta->tReciboVentaLetraOutEf))
									@php($pagadoAnterior=true)
										@foreach($tReciboVenta->tReciboVentaLetraOutEf as $key => $value)
											...
											<tr>
												...
												<td class="text-center">S/{{$value->pagado}}</td>
												<td class="text-center">S/{{$value->porPagar}}</td>
												...
												<td>
													...
													@else
														...
														<span data-urlredirect="{{url('reciboventaletra/marcarcomopagadoletrasinfe', $value->codigoReciboVentaLetraOutEf)}}" onclick="confirmarMarcarComoPagado(this)" class="btn btn-default btn-xs glyphicon glyphicon-ok" data-toggle="tooltip" data-placement="left" title="Marcar como pagado"></span>
													@endif
												</td>
											</tr>	
										@endforeach
						...
						<div class="col-md-12">
							...
							<table class="table table-striped" style="min-width: 777px;">
								<tbody>
									...
									@if(isset($tReciboVenta->tReciboVentaPagoOutEf))
										@foreach($tReciboVenta->tReciboVentaPagoOutEf as $key => $value)
											<tr>
												<td class="text-center">{{$key + 1}}.</td>
												<td class="text-center">{{$value->created_at}}</td>
												<td class="text-center">S/{{$value->monto}}</td>										
												<td class="text-right">
													<span class="btn btn-default btn-xs glyphicon glyphicon-print" data-toggle="tooltip" data-placement="left" title="Imprimir comprobante" data-urlredirect="{{url('reciboventaletra/imprimircomprobantesinfe', $value->codigoReciboVentaPagoOutEf)}}" onclick="imprimirComprobante(this)"></span>
													<span class="btn btn-danger btn-xs glyphicon glyphicon-remove" data-toggle="tooltip" data-placement="left" title="Eliminar" data-urlredirect="{{url('reciboventaletra/eliminarsinfe', $value->codigoReciboVentaPagoOutEf)}}" onclick="eliminarPago(this)"></span>
												</td>
											</tr>
										@endforeach
										<tr>
											<td class="text-center" colspan="2" style="background: rgb(228, 228, 228)"><b>Total</b></td>
											<td class="text-center" style="background: rgb(228, 228, 228)"><b>S/{{number_format($tReciboVenta->tReciboVentaPagoOutEf->sum('monto'), 2, '.', '')}}</b></td>
...
<script src="{{asset('viewResources/reciboventaletra/pagoletrasinfe.js?x='.env('CACHE_LAST_UPDATE'))}}"></script>