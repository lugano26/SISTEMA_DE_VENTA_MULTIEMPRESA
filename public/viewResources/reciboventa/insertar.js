...
$(function()
{
	...
	{
		window.open(_urlBase+'/reciboventa/imprimircomprobante/'+sessionCodigoReciboVentaTemp, '_blank');
	}
	...
	{
		localStorage.precioDivisaProducto=JSON.stringify([]);
	}
});
		...
		+'<table style="width: 100%;">'
			+'<tbody>'
				+'<tr>'
					...
					+'<td style="border: 1px dotted #594444;padding: 4px;width: 110px;text-align: center;"><b>'+($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+($('#selectDivisa').val()=='Dólares' ? (dataSelectNombreProducto[indexTemp].row.precioVentaUnitario/_tipoCambioUsd).toFixed(2) : dataSelectNombreProducto[indexTemp].row.precioVentaUnitario)+'</b></td>'
				+'</tr>'
			+'</tbody>'
		+'</table>'
	+'</div>';
...
function applyCurrency(divisa, isntLoad, updateLastOnly)
{
	var objectDivisaTemp=JSON.parse(localStorage.precioDivisaProducto);

	$('#tableProducto > tbody > tr').each(function(index, element)
	{
		$(objectDivisaTemp).each(function(key, value)
		{
			if(updateLastOnly && (key+1)!=objectDivisaTemp.length)
			{
				return true;
			}

			if((index+1)!=$('#tableProducto > tbody > tr').length && ($(element).find('input[name^=hdCodigoOficinaProducto]').val()+$(element).find('input[name^=hdNombreProducto]').val().viiInjectionEscape()==value.codigoOficinaProducto+value.nombreProducto))
			{					
				$(element).find('input[name^=hdPrecioCompraUnitarioProducto]').val(divisa=='Dólares' ? value.precioCompraUnitarioProductoDolares : value.precioCompraUnitarioProductoSoles);
				$(element).find('input[name^=hdPrecioVentaUnitarioProducto]').val(divisa=='Dólares' ? value.precioVentaUnitarioProductoDolares : value.precioVentaUnitarioProductoSoles);

				$(element).find('.tdPrecioCompraUnitarioProducto').text(divisa=='Dólares' ? 'US$'+value.precioCompraUnitarioProductoDolares : 'S/'+value.precioCompraUnitarioProductoSoles);
				$(element).find('.tdPrecioVentaUnitarioProducto').text(divisa=='Dólares' ? 'US$'+value.precioVentaUnitarioProductoDolares : 'S/'+value.precioVentaUnitarioProductoSoles);

				calcularPreciosItem(element, isntLoad, value.porcentajeTributacionProducto);
...
function agregarProducto(isntLoad, codigoOficinaProducto, codigoBarrasProducto, nombreProducto, informacionAdicionalProducto, tipoProducto, situacionImpuestoProducto, tipoImpuestoProducto, porcentajeTributacionProducto, presentacionProducto, unidadMedidaProducto, pesoGramosUnidadProducto, precioCompraUnitarioProducto, precioVentaUnitarioProducto, cantidadProducto, subTotalProducto, impuestoAplicadoProducto, precioVentaTotalProducto)
{
	...
	var existeProductoDivisa=false;
	...
	$(objectDivisaTemp).each(function(key, value)
	{
		if((codigoOficinaProducto+nombreProducto).trim().viiInjectionEscape()==(value.codigoOficinaProducto+value.nombreProducto).trim().viiInjectionEscape())
		{
			existeProductoDivisa=true;

			return false;
		}
	});

	if(!existeProductoDivisa)
	{
		var precioCompraUnitarioProductoSolesTemp=null;
		var precioVentaUnitarioProductoSolesTemp=null;
		var precioCompraUnitarioProductoDolaresTemp=null;
		var precioVentaUnitarioProductoDolaresTemp=null;

		if(codigoOficinaProducto>=parseInt(_codigoOficinaProductoExterno) && $('#selectDivisa').val()=='Dólares')
		{
			precioCompraUnitarioProductoSolesTemp=(precioCompraUnitarioProducto=='---' ? '---' : parseFloat(precioCompraUnitarioProducto*_tipoCambioUsd).toFixed(2));
			precioVentaUnitarioProductoSolesTemp=parseFloat(precioVentaUnitarioProducto*_tipoCambioUsd).toFixed(2);
			precioCompraUnitarioProductoDolaresTemp=(precioCompraUnitarioProducto=='---' ? '---' : parseFloat(precioCompraUnitarioProducto).toFixed(2));
			precioVentaUnitarioProductoDolaresTemp=parseFloat(precioVentaUnitarioProducto).toFixed(2);
		}
		else
		{
			precioCompraUnitarioProductoSolesTemp=(precioCompraUnitarioProducto=='---' ? '---' : parseFloat(precioCompraUnitarioProducto).toFixed(2));
			precioVentaUnitarioProductoSolesTemp=parseFloat(precioVentaUnitarioProducto).toFixed(2);
			precioCompraUnitarioProductoDolaresTemp=(precioCompraUnitarioProducto=='---' ? '---' : parseFloat(precioCompraUnitarioProducto/_tipoCambioUsd).toFixed(2));
			precioVentaUnitarioProductoDolaresTemp=parseFloat(precioVentaUnitarioProducto/_tipoCambioUsd).toFixed(2);
		}

		var objectDivisaTemp=JSON.parse(localStorage.precioDivisaProducto);
		
		objectDivisaTemp.push(
		{
			codigoOficinaProducto: codigoOficinaProducto,
			nombreProducto: nombreProducto.trim().viiInjectionEscape(),
			porcentajeTributacionProducto: porcentajeTributacionProducto,
			precioCompraUnitarioProductoSoles: precioCompraUnitarioProductoSolesTemp,
			precioVentaUnitarioProductoSoles: precioVentaUnitarioProductoSolesTemp,
			precioCompraUnitarioProductoDolares: precioCompraUnitarioProductoDolaresTemp,
			precioVentaUnitarioProductoDolares: precioVentaUnitarioProductoDolaresTemp
		});

		localStorage.precioDivisaProducto=JSON.stringify(objectDivisaTemp);
	}
	...
	var htmlTemp='<tr>'
		...
		+'<td class="tdPrecioCompraUnitarioProducto text-center">'+(precioCompraUnitarioProducto!='---' ? ($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/') : '')+precioCompraUnitarioProducto+'</td>'
		+'<td class="tdPrecioVentaUnitarioProducto text-center">'+($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+precioVentaUnitarioProducto+'</td>'
		...
		+'<td class="tdPrecioVentaTotalProducto text-center tdEditable" contenteditable onkeyup="onKeyUpTdPrecioVentaTotalProducto(this);" onfocus="onFocusTdPrecioVentaTotalProducto(this);" onblur="onBlurTdPrecioVentaTotalProducto(this);">'+($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+precioVentaTotalProducto+'</td>'
	...
	applyCurrency($('#selectDivisa').val(), isntLoad, true);
...
{
	$(element).text($(element).text().viiReplaceAll(($('#selectDivisa').val()=='Dólares' ? 'US\\$' : 'S/'), ''));
}

function onBlurTdPrecioVentaTotalProducto(element)
{
	$(element).text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+(!isNaN($(element).text()) ? parseFloat($(element).text()).toFixed(2) : $(element).text()));
...
function calcularPreciosItem(element, parametroCantidad, porcentajeTributacion)
{
	...
	if(parametroCantidad)
	{
		...
		$(element).find('[class^=tdSubTotalProducto]').text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+subTotalCalculado);
		$(element).find('[class^=tdImpuestoAplicadoProducto]').text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+impuestoCalculado);
		$(element).find('[class^=tdPrecioVentaTotalProducto]').text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+totalCalculado);
	...
	$(element).find('.tdPrecioVentaUnitarioProducto').text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+(totalCalculado/(cantidadProductoTemp==0 ? 1 : cantidadProductoTemp)).toFixed(2));
}
...
function calcularPreciosTotales()
{
	...
	$('#tableProducto > tbody > tr').each(function(index, element)
	{
		if(!($(element).find('input[name^=hdNombreProducto]').length))
		{
			...
			$(element).find('[class^=tdSubTotalProducto]').text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+$('#hdSubTotal').val());
			$(element).find('[class^=tdImpuestoAplicadoProducto]').text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+$('#hdImpuestoAplicado').val());
			$(element).find('[class^=tdPrecioVentaTotalProducto]').text(($('#selectDivisa').val()=='Dólares' ? 'US$' : 'S/')+$('#hdTotal').val());
...
function quitarProducto(element)
{
	...
	{
		var codigoOficinaProductoTemp=$(element).parent().parent().find('input[name^=hdCodigoOficinaProducto]').val();
		var nombreProductoTemp=$(element).parent().parent().find('input[name^=hdNombreProducto]').val();

		var objectDivisaTemp=JSON.parse(localStorage.precioDivisaProducto);

		$(objectDivisaTemp).each(function(key, value)
		{
			if((codigoOficinaProductoTemp+nombreProductoTemp).trim().viiInjectionEscape()==(value.codigoOficinaProducto+value.nombreProducto).trim().viiInjectionEscape())
			{
				objectDivisaTemp.splice(key, 1);
...
function enviarFrmInsertarReciboVenta()
{
	...
	mostrarIntruso("No olvides asignar la categoría de venta<img src=\""+_contentBase+'/img/general/sunat.jpg'+"\" style=\"border-radius: 5px;width: 100%;\"><b>Recuerda que esta venta será comunicada a la SUNAT.</b>");
...
function enviarFrmProforma()
{
	...
	confirmacion(function(){
		...
		$('#frmInsertarReciboVenta')[0].submit();
		$('#frmInsertarReciboVenta').attr('action', _urlBase+'/reciboventa/insertar');
